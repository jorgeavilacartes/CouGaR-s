...
for subset in ["train","val","test"]:
    list_subset = datasets[subset]
    labels_subset = labels[subset]

    ds_subset = DataGenerator(
        list_subset,
        labels_subset,
        order_output_model = ORDER_OUTPUT,
        batch_size = BATCH_SIZE,
    ...
    # Save test embeddings for visualization in projector
    results = model.predict(ds_subset,)
    np.savetxt(PATH_TEST.joinpath(f"embeddings-{subset}.tsv"), results, delimiter="\t")